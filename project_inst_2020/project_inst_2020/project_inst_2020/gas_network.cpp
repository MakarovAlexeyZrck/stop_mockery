...
void gas_network::create_connection(unordered_map<int, trumpet>& Pipeline_s, const unordered_map<int, ks>& Ks_s)
{
	...
	{
		auto ks_from = Ks_s.find(input_value("Введите id КС от которой делаем связь: ", 1, ks::MaxID));
		auto ks_to = Ks_s.find(input_value("Введите id КС к которой делаем связь: ", 1, ks::MaxID));
